...
- #7 管理画面ログイン認証(ID/PASS): src/core/auth/(session.ts=HMAC署名cookie, credentials.ts=固定幅HMACで
  constant-time verifyLogin, auth-config.ts=env読込[ADMIN_USERNAME/ADMIN_PASSWORD/AUTH_SESSION_SECRET base64>=32,
  TTL12h], login-throttle.ts=指数backoffのin-memoryブルートフォース対策) + src/app/lib/auth.ts(createSession/
  deleteSession/requireAdmin[DAL,cache,subをADMIN_USERNAMEと再照合]) + src/proxy.ts(=middleware改称。cookie存在のみの
  楽観redirect。/loginからの追い出しはしない=expired cookieの無限ループ回避) + src/app/login/(page/form[useActionState]/actions)
- 重要(Codex HIGH): Server ActionはPOST直叩きでproxyを迂回できる→saveCredential/deleteCredential/logout/loginの
  各actionで await requireAdmin() を冒頭(try外)で必須化。redirect()のNEXT_REDIRECT throwをtryで握り潰さない配置
- 重要(Codex HIGH): login無制限試行→LoginThrottle(username key, threshold5, base2s, max15分, 指数backoff)。
  globalThis singletonでHMR跨ぎ1インスタンス。失敗recordFailure/成功recordSuccess
- proxy注意: matcher=['/admin/:path*']のみ。/login→/admin redirectは入れない(present-but-invalid cookieで
  /login↔/admin ping-pong無限loopになるため)。__Host-prefixはdev httpでsecure付かず弾かれるので不採用、sameSite=lax
- gate全green(161 passed/1 skipped)。ブラウザ検証(Playwright): 未認証/admin→/login, 誤認証→汎用エラー,
  正認証→/admin到達, 認証済save動作, ログアウト→/login&cookie消去 全OK
- .env.local追記済: ADMIN_USERNAME=admin / ADMIN_PASSWORD=local-dev-pass / AUTH_SESSION_SECRET(base64 32B)。.env.exampleにも雛形追加
